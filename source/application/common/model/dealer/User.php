...
class User extends BaseModel
{
    ...
    public static function detail($userId, $with = ['user', 'referee'])
    {
        ...
        $data = self::get($userId, $with);
        $count = (new Withdraw())->where('user_id',$userId)->where('apply_status',40)->count();
        $data['total_money'] = $data['total_money'] + ($count*3);
        return $data;
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function grantTeamMoney($user_id, $money)
    {
        // 分销商详情
        $model = static::detail($user_id);
        if (!$model || $model['is_delete']) {
    ...
    public static function grantPoints($user_id,$points){
        //用户详情
        $model = \app\common\model\User::detail($user_id);
        if(!$model || $model['is_delete']){
            return false;
        }
        //累计用户积分
        $model->setInc('points',$points);
        Capital::add([
            'user_id' => $user_id,
            'flow_type' => 30,
            'money' => $points,
