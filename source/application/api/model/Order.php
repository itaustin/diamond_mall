...
class Order extends OrderModel
{
     ...
     * @throws \think\exception\DbException
     */
    public function onOrderPayment_($user, $order, $payType)
    {
        if ($payType == PayTypeEnum::WECHAT) {
            return $this->onPaymentByWechat_($user, $order);
        }
        return [];
    }
     ...
     * @throws \think\exception\DbException
     */
    protected function onPaymentByWechat_($user, $order)
    {
        return PaymentService::wechat_store(
            $user,
            $order['order_id'],
            $order['order_no'],
            $order['pay_price'],
    ...
    public function getOrderGoodsListByNow($goodsId, $goodsSkuId, $goodsNum)
    {
        ...
        foreach ($goodsList as &$item) {
            ...
            $item['total_points'] = bcmul($item['points'], $goods['total_num'],2);
        }
        return $goodsList;
    }
...
//            ]);
            ...
            //1号商城在发货后结算佣金
            //2号商城确认收货后结算佣金
            $goods_id = $this['goods'][0]['goods_id'];
            $mall_no = Goods::detail($goods_id)['mall_no'];
            if($mall_no === 2){
