...
class Apply extends ApplyModel
{
    ...
    public function submit($data)
    {
        ...
        if ($data['apply_status'] == '20') {
            ...
            $first = $referee->where("dealer_id",$this['user_id'])->where("level","1")->count();
            $second = $referee->where("dealer_id",$this['user_id'])->where("level","2")->count();
            $third = $referee->where("dealer_id",$this['user_id'])->where("level","3")->count();
