                            ...
                            <div class="am-form-group">
                                <div class="am-btn-group am-btn-group-xs">
                                    <a class="am-btn am-btn-default am-btn-success"
                                       href="index.php?s=/store/agent.withdraw/apply">
                                        <span class="am-icon-plus"></span> 去提现
                                    </a>
                                </div>
                            </div>
                            ...
                            <?php if (!$list->isEmpty()): foreach ($list as $item): ?>
                                <tr>
                                    ...
                                    <td class="am-text-middle">
                                        <?php if (!empty($item['real_name']) || !empty($item['mobile_phone'])): ?>
                                            <p><?= $item['real_name'] ?: '--' ?></p>
                                    ...
                                    <td class="am-text-middle">
                                        <p><span><?= $item['money'] ?> (实际提现金额为：<?= $item['money'] + 3 ?>元)(手续费3元)</span></p>
                                    </td>
                                    <td class="am-text-middle">
                                    ...
                                    <td class="am-text-middle am-text-center">
                                        ...
                                        <?php elseif ($item['apply_status'] == 40) : ?>
                                            <span class="am-badge am-badge-success">提现成功</span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="am-text-middle">
                                        <div class="tpl-table-black-operation">
                                            <?php if (in_array($item['apply_status'], [10, 20])) : ?>
                                                <?php if (checkPrivilege('agent.withdraw/submit')): ?>
                                                ...
                                                <?php if ($item['apply_status'] == 20) : ?>
                                                    <?php if (checkPrivilege('agent.withdraw/money')): ?>
                                                ...
                                                <?php if ($item['apply_status'] == 20 && $item['pay_type']['value'] == 10) : ?>
                                                    <?php if (checkPrivilege('agent.withdraw/wechat_pay')): ?>
...
<script id="tpl-dealer-withdraw" type="text/template">
    <div class="am-padding-top-sm">
        <form class="form-dealer-withdraw am-form tpl-form-line-form" method="post"
              action="<?= url('agent.withdraw/submit') ?>">
...
<script>
    $(function () {
        ...
        $('.j-money').click(function () {
            var id = $(this).data('id');
            var url = "<?= url('agent.withdraw/money') ?>";
            layer.confirm('确定提现成功吗？', {title: '友情提示'}, function (index) {
        ...
        $('.j-wechat-pay').click(function () {
            var id = $(this).data('id');
            var url = "<?= url('agent.withdraw/wechat_pay') ?>";
