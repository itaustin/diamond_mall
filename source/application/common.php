...
/**
 * Powered By 朔州市佐威网络科技有限公司
 * Author: （产品研发部）Paul Austin
 * @param $num
 * @return float|int
 * 计算数字的百分
 */
function bTon($num)
{
    return bcdiv($num, 100, 5);
}

/**
 * 检测微信的版本号
 */
function wechat_version() {
 ...
 * @return string
 */
function zuowey_hash($password)
{
    return md5(md5($password) . 'zuowey_salt_SmTRx');
}
 ...
 * @param string $type
 */
function log_write($value, $type = 'zuowey-info')
{
...
function export_excel($fileName, $tileArray = [], $dataArray = [])
{
    ...
    fwrite($fp, chr(0xEF) . chr(0xBB) . chr(0xBF));// 转码 防止乱码(比如昵称)
